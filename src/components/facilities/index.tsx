...
export function FacilitiesManagement() {
  ...
  const facilities: OrganizationItem[] = Array.isArray(facilitiesData)
    ? facilitiesData
    : [];
  ...
  useEffect(() => {
    async function loadWards() {
      ...
      const uniqueProvinceCodes = [
        ...new Set(facilities.map((f) => f.provinceCode)),
      ];
        ...
        try {
          ...
          if (provinceDetail && provinceDetail.communes) {
            ...
            for (const commune of provinceDetail.communes) {
              ...
              const facilityWithWard = facilities.find(
  ...
  const filteredFacilities = facilities.filter((facility) => {
    ...
    const matchesSearch =
      removeVietnameseTones(facility.name).includes(normalizedQuery) ||
    ...
    const matchesProvince =
      filterProvince === "all" || facility.provinceCode === filterProvince;
  ...
  const {
    ...
    paddedItems,
    setCurrentPage,
  } = usePagination(filteredFacilities, ITEMS_PER_PAGE);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Quản lý các cơ sở đào tạo trong hệ thống ({facilities.length} cơ sở)
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          ...
          <p className="text-2xl font-bold text-primary-600">
            {facilities.reduce((sum, f) => sum + (f.studentCount || 0), 0)}
          </p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          <p className="text-sm text-gray-500">Tổng giáo viên</p>
          <p className="text-2xl font-bold text-amber-600">
            {facilities.reduce((sum, f) => sum + (f.teacherCount || 0), 0)}
          </p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          <p className="text-sm text-gray-500">Số tỉnh/TP</p>
          <p className="text-2xl font-bold text-purple-600">
            {new Set(facilities.map((f) => f.provinceCode)).size}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {paddedItems.map((facility, index) => {
          ...
          const statusInfo =
            facilityStatusConfig[facility.status] ||
            facilityStatusConfig.inactive;
          const fullAddress = getFullAddress(facility);
          const provinceName = getProvinceName(facility.provinceCode);
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <span className="text-sm font-medium text-gray-900">
                        {managerMap[facility.managerId] || "Chưa có quản lý"}
                      </span>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                type="email"
                name="email"
                placeholder="facility@vietsign.edu.vn"
                className="w-full px-4 py-2.5 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-primary-500 transition-all"
                required
              />
            </div>
