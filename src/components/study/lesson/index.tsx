...
import { mockClasses, getClassById } from "@/src/data/classesData";
import {
  getLessonById,
  getStepsByLessonId,
  LessonItem,
  StepItem,
} from "@/src/data/lessonsData";
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (foundLesson) {
          const lessonSteps = getStepsByLessonId(lessonId);
        ...
        <div className="divide-y divide-gray-100">
          {steps.map((step, index) => {
            ...
            return (
              <Link
                key={step.id}
                href={`/study/${classId}/${lessonId}/${step.id}`}
